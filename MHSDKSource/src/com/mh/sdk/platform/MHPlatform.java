...
import com.mh.sdk.MHSDKBaseActivity;
	...
	public int mhPlatformLogin(Context context,MHPlatformCallBack callBack){
		if(MHPlatformConfig.isInitSDK){
			...
			if (isAutoLogin && (username != null && !"".equals(username))) {
				...
				loginService.userLogin(username, password,context.getResources().getString(MHSDKBaseActivity.getResourcesIdByName(context,MHSDKBaseActivity.STRING,"fuid")));
			} else {
...
//			return MHPlatformCallBackCode.LOGINERROR;
	 ...
	 * @return
	 */
	public int mhPlatformPayment(Context context, String subject, String gameid, String serverid,String mark, MHPlatformCallBack callBack){
		if(MHPlatformConfig.isInitSDK){
			if(MHPlatformConfig.isLogin){
				this.context = context;
				this.setCallBackListener(callBack);
